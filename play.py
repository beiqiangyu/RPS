...
while True:
    ...
    if(k==ord("s")or start):
        ...
        if (result_index == 0):
            ...
            move="scissors"
        if(move!="none"):
            cv.putText(frameWithoutText,"Your's Move : "+move,(320,50),cv.FONT_HERSHEY_PLAIN,1.5,(0,0,255),2)
            computer_move_name = np.random.choice(['rock', 'paper', 'scissors'])
            # cv.putText(frameWithoutText,"Computer's Move : "+ computer_move_name,(50,450),cv.FONT_HERSHEY_PLAIN,1,(0,0,255),2)
            # winner=calculate_winner(move, computer_move_name)
            # cv.putText(frameWithoutText,"winner : "+ winner,(350,150),cv.FONT_HERSHEY_PLAIN,1,(0,0,255),2)
        else:
            cv.putText(frameWithoutText,"waiting of posture",(350,50),cv.FONT_HERSHEY_PLAIN,1,(0,0,255),2)
        ...
        cv.putText(frameWithText,"press 's' to start",(50,50),cv.FONT_HERSHEY_PLAIN,1,(0,0,255),2)
